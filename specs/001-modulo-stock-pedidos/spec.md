...
- **RF-016a** (RF-16): El Precio de Venta mostrado en la pantalla de artículos es informativo y NO editable: la fuente de verdad es el cálculo de RF-016 en el servidor, de modo que un cliente que no ejecute ningún recálculo no puede alterar el precio grabado. *(Implementado.)* — *pendiente de implementación*: además, el sistema DEBE **recalcularlo de forma interactiva** a medida que el usuario edita el Precio de Costo o el Margen, sin necesidad de grabar ni recargar.
